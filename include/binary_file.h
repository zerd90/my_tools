...
struct DataBlock
{
    ...
    DataBlock() : length(0) {}
    explicit DataBlock(uint64_t len) : data(new uint8_t[len]) { length = len; }
...
public:
    BinaryReader() { read_buffer = new uint8_t[buffer_size]; }

    explicit BinaryReader(std::string &fn) : BinaryReader() { open(fn); }
